...
function x = subprocess(fitness,i,xmax,xmin,x,x_gb,cmax,cmin,dime,wg,wl,wr,f_max,k1,k2)
xi_1 = x(i,:) + wg * (x_gb - x(i,:));
xi_2 = xi_1 + wl * (x_gb(i,:) - x(i,:));
% 计算概率
if f_max == -1
    p_xi_3 = 1;
    p_xi_4 = 1;
...
x_candi=[xi_1;xi_2;xi_3;xi_4];
ry=[];
rx=[];
for i=1:4
    xi_candi=x_candi(i,:);
    ry(i)=fitness(xi_candi);
    rx(i,:)=xi_candi;
end
